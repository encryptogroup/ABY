/**
 ...
 \brief		Private Set Intersection Test class implementation using 2D Cuckoo hashing.
 */
...
#include "common/psi_2D_CH_circuit.h"
...
int32_t read_test_options(int32_t* argcp, char*** argvp, e_role* role,
        ...
        uint32_t* n_partner_eles, int* stash, uint32_t* maxbin, uint32_t* nhashfuns, uint32_t * threshold = 0) {
    ...
    parsing_ctx options[] = {
        ...
        { (void*) threshold, T_NUM, "q", "Threshold of not showing the cardinality, default : 0", true, false},
        ...
        { (void*) nhashfuns, T_NUM, "h", "Number of hash functions for the hashing schemes, default: 2", false, false}
    };
...
int main(int argc, char** argv) {
    ...
    e_role role;
    uint32_t bitlen = 32, neles = 2, secparam = 128, nthreads = 1, partner_neles = 0,
            server_neles, client_neles;
    uint16_t port = 7766;
    ...
    int stash = -1;
    uint32_t maxbin = 0;
    uint32_t nhashfuns = 2;

    read_test_options(&argc, &argv, &role, &bitlen, &neles, &secparam, &address,
            &port, &test_op, &epsilon, &sharing, &nthreads, &partner_neles, &stash,
            &maxbin, &nhashfuns, &threshold);
    ...
    //if(useyao) {
    test_2D_CH_circuit(role, (char*) address.c_str(), port, seclvl, server_neles, client_neles, bitlen,
            epsilon, nthreads, mt_alg, sharing, stash, maxbin, nhashfuns, threshold);
    /*} else {
