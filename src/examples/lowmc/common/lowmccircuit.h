...
/* versions of the lowmc cipher: short and long term security */
enum LowMCVersion {
	STLowMC = 0, LTLowMC = 1
};
...
int32_t test_lowmc_circuit(e_role role, char* address, uint32_t nvals, uint32_t nthreads, e_mt_gen_alg mt_alg, e_sharing sharing, uint32_t statesize, uint32_t keysize,
		uint32_t sboxes, uint32_t rounds, uint32_t maxnumgates, crypto* crypt);
int32_t test_lowmc_circuit(e_role role, char* address, uint32_t nvals, uint32_t nthreads, e_mt_gen_alg mt_alg, e_sharing sharing, LowMCParams* param, uint32_t maxgates,
		crypto* crypt);
