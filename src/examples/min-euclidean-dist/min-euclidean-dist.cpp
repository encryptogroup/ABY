...
int32_t read_test_options(int32_t* argcp, char*** argvp, e_role* role, uint32_t* bitlen, uint32_t* nvals, uint32_t* dim, uint32_t* secparam, string* address, uint16_t* port, int32_t* test_op) {
...
int main(int argc, char** argv) {
	...
	read_test_options(&argc, &argv, &role, &bitlen, &nvals, &dim, &secparam, &address, &port, &test_op);

	seclvl seclvl = get_sec_lvl(secparam);

	test_min_eucliden_dist_circuit(role, (char*) address.c_str(), seclvl, nvals, dim, nthreads, mt_alg, S_ARITH, S_YAO);
