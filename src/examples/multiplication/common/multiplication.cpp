/**
 ...
 \brief		Implementation of the multiplication of two numbers using ABY Framework.
 */
...
int32_t test_multiplication_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl,
	...
	*/
	...
	s_out = BuildMultiplicationCircuit(s_A, s_B,
	...
	*/
	...
	std::cout << "Testing multiplication computation in " << get_sharing_name(sharing)
				<< " sharing: " << std::endl;
	std::cout << "\nValue A:\t" << a_val;
	std::cout << "\nValue B:\t" << b_val;
...
share* BuildMultiplicationCircuit(share *s_a, share *s_b,
		ArithmeticCircuit *ac) {

	share* out;

	/** Calling the multiplication gate in the Arithmetic circuit.*/
	out = ac->PutMULGate(s_a, s_b);
