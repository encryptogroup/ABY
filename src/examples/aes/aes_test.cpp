...
int32_t read_test_options(int32_t* argcp, char*** argvp, e_role* role, uint32_t* bitlen, uint32_t* nvals,
		...
		bool* use_vec_ands, bool* ex_calc, bool* client_only) {
	...
	parsing_ctx options[] = { { (void*) &int_role, T_NUM, "r", "Role: 0/1", true, false },
			...
			{ (void*) ex_calc, T_FLAG, "x", "Calculate the key expansion during the SFE, default: false", false, false },
...
int main(int argc, char** argv) {
	...
	read_test_options(&argc, &argv, &role, &bitlen, &nvals, &secparam, &address, &port, &sharing, &verbose, &nthreads, &use_vec_ands, &ex_calc, &client_only);

	seclvl seclvl = get_sec_lvl(secparam);

	test_aes_circuit(role, address, port, seclvl, nvals, nthreads, mt_alg, sharing, verbose, use_vec_ands, ex_calc, client_only);
