...
int32_t read_test_options(int32_t* argcp, char*** argvp, e_role* role, uint32_t* bitlen, uint32_t* nvals,
		...
		bool* use_vec_ands, bool* extend_in_sfe, bool* client_only) {
	...
	parsing_ctx options[] = { { (void*) &int_role, T_NUM, "r", "Role: 0/1", true, false },
			...
			{ (void*) extend_in_sfe, T_FLAG, "x", "Calculate the key expansion during the SFE, default: false", false, false },
...
int main(int argc, char** argv) {
	...
	bool extend_in_sfe = false;
	bool client_only = false;
	e_mt_gen_alg mt_alg = MT_OT;

	e_sharing sharing = S_BOOL;

	read_test_options(&argc, &argv, &role, &bitlen, &nvals, &secparam, &address, &port, &sharing, &verbose, &nthreads, &use_vec_ands, &extend_in_sfe, &client_only);

	seclvl seclvl = get_sec_lvl(secparam);

	test_aes_circuit(role, address, port, seclvl, nvals, nthreads, mt_alg, sharing, verbose, use_vec_ands, extend_in_sfe, client_only);
